...
import com.nolson.plugins.solanamobilewallet.data.Ed25519KeyRepository
import com.nolson.plugins.solanamobilewallet.usecase.ClientTrustUseCase
import com.nolson.plugins.solanamobilewallet.usecase.SendTransactionsUseCase
import com.nolson.plugins.solanamobilewallet.usecase.SolanaSigningUseCase
...
class SolanaMobileWallet(application: Application) : AndroidViewModel(application) {

    private val _mobileWalletAdapterServiceEvents = MutableStateFlow<MobileWalletAdapterServiceRequest>(MobileWalletAdapterServiceRequest.None)
    val mobileWalletAdapterServiceEvents = _mobileWalletAdapterServiceEvents.asSharedFlow() // expose as event stream, rather than a stateful object

    private var clientTrustUseCase: ClientTrustUseCase? = null
    private var scenario: Scenario? = null

    fun processLaunch(intent: Intent?, callingPackage: String?, activity: Activity): Boolean {
        if (intent == null) {
    ...
    fun authorizeDapp(
        ...
        viewModelScope.launch {
            Log.d("REQUEST", "co-routine started")
            if (authorized) {
                Log.d("Co-routine authorized block", "TRUE")

                val keypair = Ed25519KeyRepository(activity.applicationContext).generateKeypair()
    ...
    fun signPayloadsSimulateSign(request: MobileWalletAdapterServiceRequest.SignPayloads, activity: Activity) {
        if (rejectStaleRequest(request)) {
            return
        }

        viewModelScope.launch {
            val keypair = Ed25519KeyRepository(activity.applicationContext).getKeypair(request.request.authorizedPublicKey)
    ...
    fun signAndSendTransactionsSimulateSign(request: MobileWalletAdapterServiceRequest.SignAndSendTransactions) {
        viewModelScope.launch {
            val keypair = Ed25519KeyRepository(Application()).getKeypair(request.request.publicKey)
            check(keypair != null) { "Unknown public key for signing request" }
    ...
    companion object {
        private val TAG = SolanaMobileWallet::class.simpleName
